...
//LED_ALPHABET_CNT (sizeof(digits)/ sizeof( digits[0]))
//#define LED_BLANK  LED_ALPHABET_CNT

#define LED_DIGIT_COUNT ((sizeof led_cnt/ sizeof led_cnt[0]))

static void led_init_segment_pins(void);
static void led_init_anodes_pins(void);
static void led_select_active_pos(uint8_t n);
static void led_set_digit(uint8_t d);
...
/* ~ */		(SEG_A ),
/* _ */		( SEG_D),
/*deg */	(SEG_A + SEG_B +   SEG_F + SEG_G),
 

};

const uint8_t LED_ALPHABET_CNT = (sizeof(digits)/ sizeof( digits[0]));
const uint8_t LED_BLANK = 11;
...
void TIMER4_isr(void) __interrupt(IRQ_TIM4)
{
	...
	led_set_digit(LED_BLANK); /* Helps not to have ghosting when swithcing digits */
	led_select_active_pos(multiplex_pos);
	led_set_digit(digit);

	multiplex_pos++;

	if (multiplex_pos > LED_DIGIT_COUNT-1)
	{
		multiplex_pos=0;
	}
...
static void led_set_digit(uint8_t d)
{
	uint8_t segments;
	uint8_t AB,CDE,FG;
	if (d >= LED_ALPHABET_CNT)
	{
		d = LED_ALPHABET_CNT-1;
	}
	segments = digits[d];
	...
	LED_SEG_AB_PORT->ODR  = ((LED_SEG_AB_PORT->ODR  | SEG_AB_HW))  & (uint8_t)(~AB);
	LED_SEG_CDE_PORT->ODR = ((LED_SEG_CDE_PORT->ODR | SEG_CDE_HW)) & (uint8_t)(~CDE);
